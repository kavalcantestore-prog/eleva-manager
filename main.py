...
def social_media_select(request: Request):
    user = require_user(request)
    conn = get_db()
    clients = conn.execute("SELECT id, name, company, email, phone FROM clients ORDER BY name").fetchall()
    conn.close()
    return templates.TemplateResponse("social_media_select.html", {
        "request": request,
        "user": dict(user),
        "clients": [dict(c) for c in clients]
    })
...
@app.get("/social-media/{client_id}", response_class=HTMLResponse)
def social_media_page(request: Request, client_id: int):
    user = require_user(request)
    conn = get_db()
    ...
    posts = conn.execute("SELECT * FROM scheduled_posts WHERE client_id=? ORDER BY created_at DESC", (client_id,)).fetchall()
    conn.close()
...
def social_media_detalhes(request: Request, client_id: int, pid: int):
    user = require_user(request)
    conn = get_db()
    post = conn.execute("SELECT * FROM scheduled_posts WHERE id=? AND client_id=?", (pid, client_id)).fetchone()
    conn.close()
...
@app.post("/social-media/{client_id}/posts/{pid}/editar")
def social_media_editar(request: Request, client_id: int, pid: int, platform: str = Form(...), content: str = Form(...), image_url: str = Form(""), responsible: str = Form(""), scheduled_for: str = Form(""), posted_at: str = Form("")):
    user = require_user(request)
    conn = get_db()
    ...
    conn.execute(
        "UPDATE scheduled_posts SET platform=?, content=?, image_url=?, responsible=?, scheduled_for=?, posted_at=? WHERE id=?",
        (platform, content, image_url or None, responsible, scheduled_for or None, posted_at or None, pid)
    )
...
@app.post("/api/social-media/{client_id}/posts/{pid}/status")
async def social_media_status_api(request: Request, client_id: int, pid: int):
    user = require_user(request)
    data = await request.json()
    status = data.get("status", "")
    ...
    conn = get_db()
    post = conn.execute("SELECT content FROM scheduled_posts WHERE id=? AND client_id=?", (pid, client_id)).fetchone()
...
@app.post("/social-media/{client_id}/posts/{pid}/deletar")
def social_media_deletar(request: Request, client_id: int, pid: int):
    user = require_user(request)
    conn = get_db()
    row = conn.execute("SELECT content FROM scheduled_posts WHERE id=? AND client_id=?", (pid, client_id)).fetchone()

    if not row:
        raise HTTPException(status_code=404, detail="Post não encontrado")

    conn.execute("DELETE FROM scheduled_posts WHERE id=?", (pid,))
    log_action(conn, user, "removeu", "post social media", row['content'][:50])
    conn.commit(); conn.close()
    return RedirectResponse(f"/social-media/{client_id}", status_code=302)
